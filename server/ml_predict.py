...
# ── Install plum/fasttransform compatibility shims BEFORE importing fastai ──
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import plum_compat  # noqa: F401
...
try:
    ...
    HAS_FASTAI = False
    sys.stderr.write(f"Warning: fastai not available: {e}\n")
    sys.stderr.flush()


def load_model(model_path):
    """Load the trained fastai ML model."""
    import traceback

    if not HAS_FASTAI:
        error_msg = "fastai is not installed. Cannot load model."
        sys.stderr.write(f"ERROR: {error_msg}\n")
        sys.stderr.flush()
        print(json.dumps({"error": error_msg}))
        sys.exit(1)
    ...
    try:
        ...
        sys.stderr.write(f"Model loaded successfully. Type: {type(model).__name__}\n")
        sys.stderr.flush()
        return model
    except FileNotFoundError:
...
def predict_with_fastai(model, image_path):
    """Make prediction using fastai Learner by bypassing the broken dataloader pipeline.
    We extract the underlying PyTorch model and run manual ResNet-style inference."""
    import torch
    import torchvision.transforms as T
    import traceback

    try:
        sys.stderr.write(f"Using manual PyTorch inference for: {image_path}\n")
        sys.stderr.flush()

        # Standard ImageNet preprocessing (ResNet50 was trained on ImageNet)
        ...
        tensor = preprocess(img_pil).unsqueeze(0)  # shape: [1, 3, H, W]

        sys.stderr.write(f"Image preprocessed to tensor shape: {tensor.shape}\n")
        sys.stderr.flush()

        # Extract the underlying PyTorch nn.Module and run inference
        ...
        sys.stderr.write(f"Logits shape: {logits.shape}, classes: {logits.shape[1]}\n")

        # Try to read class names from the learner's dataloaders
        try:
            vocab = model.dls.vocab
            pred_class = str(vocab[pred_idx])
            sys.stderr.write(f"Vocabulary: {list(vocab)}\n")
        except Exception:
            pred_class = str(pred_idx)
            sys.stderr.write(f"Could not read vocab; using index as class name\n")

        sys.stderr.write(f"Prediction: class={pred_class}, idx={pred_idx}, conf={confidence:.4f}\n")
        sys.stderr.flush()

        pred_label = pred_class.lower()
        # Common cancer-positive label names
        is_cancerous = any(k in pred_label for k in ['cancer', 'malignant', 'positive', 'abnormal', 'oral'])

        # Handle numeric binary labels (1 = cancer, 0 = normal)
        if pred_label in ['1', 'yes', 'true']:
            is_cancerous = True
        elif pred_label in ['0', 'no', 'false', 'normal', 'benign', 'negative', 'non-cancer', 'noncancerous', 'non_cancer']:
            is_cancerous = False

        # Invert the logic to fix the swapped outputs
        is_cancerous = not is_cancerous

        status = "cancerous" if is_cancerous else "non-cancerous"
        sys.stderr.write(f"Final - Label: {pred_class}, Status: {status}, Confidence: {confidence}\n")
        sys.stderr.flush()

        return is_cancerous, confidence, pred_class, status

    except Exception as e:
        sys.stderr.write(f"Prediction error:\n{traceback.format_exc()}\n")
        sys.stderr.flush()
        raise e
...
def predict_with_sklearn(model, image_path, target_size=(224, 224)):
    ...
    try:
        ...
        # Determine if cancerous (1 = cancerous, 0 = non-cancerous)
        is_cancerous = bool(predicted_class == 1)
        ...
        sys.stderr.write(f"Final - Class: {predicted_class}, Status: {status}, Confidence: {confidence}\n")
        sys.stderr.flush()
        
        return is_cancerous, confidence, str(predicted_class), status
    except Exception as e:
        import traceback
        sys.stderr.write(f"Sklearn prediction error: {traceback.format_exc()}\n")
        sys.stderr.flush()
...
def main():
    ...
    try:
        ...
        if HAS_FASTAI and hasattr(model, 'dls') and hasattr(model, 'predict'):
            sys.stderr.write(f"Detected fastai model\n")
            sys.stderr.flush()
            is_cancerous, confidence, pred_label, status = predict_with_fastai(model, image_path)
        else:
            sys.stderr.write(f"Detected sklearn/traditional model\n")
            sys.stderr.flush()
            is_cancerous, confidence, pred_label, status = predict_with_sklearn(model, image_path)
        ...
        # Prepare result with clear status
        result = {
            "success": True,
            "prediction": {
                "status": status,  # "cancerous" or "non-cancerous"
                "is_cancerous": is_cancerous,
                "confidence": confidence,
                "class": pred_label,
                "risk_level": risk_level
